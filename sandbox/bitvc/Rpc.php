...
class BitVC_Rpc {
	...
	public function request($method, $url, $params) {
		...
		// Headers
		$headers = array('User-Agent: BitVCPHP/v1');
		...
		if ($method == 'get') {
			...
			switch ($authenticationClass) {
				...
				case 'BitVC_ApiKeyAuthentication' :
					//BitVC POST请求加密流程
					ksort($params);
					$sign = "";
					...
					break;
				default :
					throw new BitVC_Exception("Invalid authentication mechanism");
					break;
			}
		...
		// CURL options
		curl_setopt($ch, CURLOPT_URL, substr(BitVCBase::WEB_BASE, 0, -1) . $url);
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
		curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);

		// $curlOpts[CURLOPT_URL] = substr(BitVCBase::WEB_BASE, 0, -1) . $url;
		...
		if (isset($json -> error)) {
			throw new BitVC_Exception($json -> error, $response['statusCode'], $response['body']);
		} else if (isset($json -> errors)) {
			throw new BitVC_Exception(implode($json -> errors, ', '), $response['statusCode'], $response['body']);
		}
